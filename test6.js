...
function switchTemplateTab(type) {
  document.getElementById('tpl-panel-email').style.display = type==='email' ? 'block' : 'none';
  document.getElementById('tpl-panel-sms').style.display   = type==='sms'   ? 'block' : 'none';
  document.getElementById('btn-tpl-email').className = 'sub-tab ' + (type==='email'?'active':'inactive');
  document.getElementById('btn-tpl-sms').className   = 'sub-tab ' + (type==='sms'?'active':'inactive');
  lucide.createIcons();
}
const RATE = 0.049;
